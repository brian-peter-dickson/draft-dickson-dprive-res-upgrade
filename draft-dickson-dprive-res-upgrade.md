...
While new privacy standards have been developed (DNS over TLS (DoT), and DNS over HTTPS (DoH)), their deployment is hampered by the inability to seamlessly upgrade from regular DNS to those, on the same resolver's IP address.
Resolvers may not have globally unique IP addresses and may not have their own fully qualified domain names (FQDNs), which is the root of the upgrade problem.

The motivation for this work is to facilitate discovery of resolvers. The methods are intended to support independent operation of intermediaries, incremental upgrades and deployments, and authentication of discovered information. The intent is to maximize the ability to achieve client privacy (e.g. via DoT or DoH), to verify the privacy status, to identify problems caused by incompatible network configurations (e.g. overlapping NAT addresses/scopes). A secondary goal is to  provide a "DNS traceroute" functionality if one or more forwarders are used.
